...
int inode_write(struct unix_filesystem *u, uint16_t inr, struct inode *inode) {
  ...
  // initialize all fields of the inode
  toBeRead[posOfInode] = inode->i_mode;
  toBeRead[posOfInode + 1] = inode->i_nlink & (inode->i_uid << 8);
  toBeRead[posOfInode + 2] = inode->i_gid & (inode->i_size0 << 8);
  toBeRead[posOfInode + 3] = inode->i_size1;

  for (int i = 0; i < ADDR_SMALL_LENGTH; ++i) {
    toBeRead[posOfInode + 4 + i] = inode->i_address[i];
  }

  toBeRead[posOfInode + ADDR_SMALL_LENGTH + 4] = inode->i_atime[0];
  toBeRead[posOfInode + ADDR_SMALL_LENGTH + 5] = inode->i_atime[1];
  toBeRead[posOfInode + ADDR_SMALL_LENGTH + 6] = inode->i_mtime[0];
  toBeRead[posOfInode + ADDR_SMALL_LENGTH + 7] = inode->i_mtime[1];
